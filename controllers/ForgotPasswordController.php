...
class ForgotPasswordController extends TutorController{
	...
	*/
	function processForgot($f3){
		...
		if ($f3->exists('POST.isPost')){
			...
			$this->users->getbyArray(array('email=? and position=?', $email, $position));
			...
			if ($this->users->email != ''){
				...
				$this->users->editByArray(array('email=? and position=?', $email, $position));
				...
				$f3->set('err_message', 'Invalid email for position '.$position);
			}
		}
